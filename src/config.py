...
1. **分析用户输入**：解析用户输入内容，提取核心需求或关键点。  
2. **结合历史对话**：参考历史对话内容以补充上下文信息，更准确判断用户意图。  
3. **匹配文章章节目录**：将用户需求与文章章节目录的结构进行比对，确定最相关的章节。  
4. **返回结果**：最终返回一个 JSON 对象，其中包括最相关的章节标题。

# Steps

1. 提取用户输入中的核心需求和关键点。  
2. 根据历史会话内容，整合上下文信息，进一步明确用户的意图。  
3. 在给定的 `{agenda_dict}` 章节目录中，匹配到与用户需求最相关的章节标题。  
4. 构造 JSON 格式的输出结构，其中包含两个字段：  
   - `"title"`：匹配到的章节标题， list 格式。
""",
